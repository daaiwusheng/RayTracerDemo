...
Cartesian3 Quaternion::getEulerAngles()
{
    Cartesian3 result;

    result[0] = std::atan2(2 * (coords[1] * coords[1] * coords[2] + coords[3] * coords[0]), coords[3] * coords[3] - coords[0] * coords[0] - coords[1] * coords[1] + coords[2] * coords[2]);
    result[1] = std::asin(-2 * (coords[0] * coords[2] - coords[3] * coords[1]));
    result[2] = std::atan2(2 * (coords[0] * coords[1] + coords[3] * coords[2]), coords[3] * coords[3] + coords[0] * coords[0] - coords[1] * coords[1] - coords[2] * coords[2]);
    return result;
}


void Quaternion::setEulerAngles(const Cartesian3 & angles){

      auto angle = angles[0] / 2.f;
      ...
      coords[0] = sr * cpcy - cr * spsy;
      coords[1] = cr * spcy + sr * cpsy;
      coords[2] = cr * cpsy - sr * spcy;
      coords[3] = cr * cpcy + sr * spsy;
}
